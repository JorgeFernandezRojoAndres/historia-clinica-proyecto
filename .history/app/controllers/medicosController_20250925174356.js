...
exports.verAgenda = (req, res) => {
    ...
    // 📌 Diferenciar validación según rol
    if (usuario.role === 'secretaria') {
        console.log(`✅ Secretaria accediendo a agenda del médico ${idMedico} en clínica ${idClinica}`);
        return continuarConAgenda(idMedico, fechaSeleccionada, usuario, res);
    }

    // 📌 Solo para médicos: validar asociación médico-clínica
    ...
    db.query(sqlVerificarClinica, [idMedico, idClinica], (error, resultados) => {
        ...
        continuarConAgenda(idMedico, fechaSeleccionada, usuario, res);
    });
};

// 🔹 Extraer lógica de turnos a una función común
function continuarConAgenda(idMedico, fechaSeleccionada, usuario, res) {
