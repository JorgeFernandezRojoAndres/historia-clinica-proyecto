...
exports.delete = (req, res) => {
    ...
    const sqlVerificar = 'SELECT estado FROM citas WHERE idCita = ?';
    db.query(sqlVerificar, [id], (error, results) => {
        if (error) return res.status(500).send('Error al verificar la cita');
        if (results.length === 0) return res.status(404).send('Cita no encontrada');

        if (results[0].estado === 'Completado') {
            return res.status(403).send('No se puede eliminar una cita completada.');
        }

        db.query('DELETE FROM citas WHERE idCita = ?', [id], (error2) => {
            if (error2) return res.status(500).send('Error al eliminar la cita');
            res.redirect('/citas');
        });
    });
};
...
exports.createCita = (req, res) => {
    ...
    // 🔹 Validar si la fecha corresponde a un día no laborable
    const fechaSolo = new Date(fechaHora).toISOString().split("T")[0];
    const sqlNoLaborable = "SELECT * FROM dias_no_laborables WHERE fecha = ?";

    db.query(sqlNoLaborable, [fechaSolo], (errNoLab, noLabRows) => {
        if (errNoLab) {
            console.error("Error al verificar día no laborable:", errNoLab);
            return res.status(500).send("Error al verificar día no laborable.");
        }

        if (noLabRows.length > 0) {
            console.log(`⛔ Intento de agendar cita en día no laborable: ${fechaSolo}`);
            return res.status(400).send(`No se pueden crear citas el ${fechaSolo}. Día no laborable: ${noLabRows[0].descripcion}`);
        }

        // 🔹 Validar si el médico está de vacaciones
        const sqlVacaciones = `
        ...
        db.query(sqlVacaciones, [sanitizedIdMedico, fechaSolo], (errVac, vacRows) => {
            if (errVac) {
            ...
            if (vacRows.length > 0) {
                console.log(`⛔ Intento de agendar cita mientras el médico ${sanitizedIdMedico} está de vacaciones`);
                return res.status(400).send(`No se pueden crear citas en esta fecha. El médico está de vacaciones del ${vacRows[0].fechaInicio} al ${vacRows[0].fechaFin}`);
            }

            // 👉 Si no es feriado ni vacaciones, continuar con el insert
            const sqlCita = `
...
exports.confirmarPendientes = (req, res) => {
    ...
    console.log("🟢 Body recibido en confirmarPendientes:", req.body);
    ...
    db.query(sql, params, (error, result) => {
        if (error) {
            ...
            // devolver error como JSON si es fetch
        ...
        // Si es fetch (AJAX), responder JSON
        if (req.xhr || req.headers.accept.includes('application/json')) {
            return res.json({
                mensaje: idCita
        ...
        return res.redirect('/citas?estado=Pendiente');
    });
};
exports.cancelarCita = (req, res) => {
    ...
    if (user.role === "paciente") {
        // Solo puede cancelar sus turnos
        const sqlVerificar = "SELECT * FROM citas WHERE idCita = ? AND idPaciente = ?";
        db.query(sqlVerificar, [idCita, user.id], (err, results) => {
            if (err) return res.status(500).send("Error al verificar la cita");
            if (results.length === 0) return res.status(403).send("No autorizado");

            const sql = 'UPDATE citas SET estado = "Cancelada" WHERE idCita = ?';
            db.query(sql, [idCita], (err2) => {
                if (err2) return res.status(500).send("Error al cancelar la cita");
                res.redirect("/turnos/mis-turnos"); // 🔹 vuelve a sus turnos
            });
        });
    } else {
