document.addEventListener('DOMContentLoaded', function() {
    ...
    window.abrirAgenda = function() {
      const clinicaSeleccionada = sessionStorage.getItem("clinicaSeleccionada") || localStorage.getItem("clinicaSeleccionada");
      if (!clinicaSeleccionada) {
        alert("Debe seleccionar una clínica antes de ver la agenda.");
        return;
      }
      window.open(`/medicos/${user.id}/agenda?clinica=${clinicaSeleccionada}`, 'AgendaMedico', 'width=800,height=600,resizable=yes,scrollbars=yes');
    };
